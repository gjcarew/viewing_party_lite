...
RSpec.describe 'new user party page', type: :feature do
  describe 'As a visitor' do
    describe 'When I visit the new user party page (/users/:user_id/movies/:movid_id/viewing-party/new) ' do
      before :each do
        @user = create(:user)
        allow_any_instance_of(UserPartiesController).to receive(:current_user).and_return(@user)
        allow_any_instance_of(UsersController).to receive(:current_user).and_return(@user)
      end

      it 'I can click create a viewing party and be brought to the right page', :vcr do
        visit '/dashboard/movies/550'
        click_button('Create a Viewing Party')

        expect(current_path).to eq('/dashboard/movies/550/viewing-party/new')
      end 
      ...
      it 'I should see the name of the movie title rendered above a form with the following fields: duration with default value, date, start time, checkboxes next to each user we want to invite and a create party button', :vcr do
        ...
        visit '/dashboard/movies/550/viewing-party/new'
      ...
      it "When the party is created, user_parties are created for each attendee", :vcr do
        ...
        allow_any_instance_of(UsersController).to receive(:current_user).and_return(user2)
        visit user_path
        expect(page).to have_content('Fight Club')

        allow_any_instance_of(UsersController).to receive(:current_user).and_return(user3)
        visit user_path
        expect(page).to have_content('Fight Club')

        allow_any_instance_of(UsersController).to receive(:current_user).and_return(user4)
        visit user_path
    ...
    context 'Sad path testing' do
      ...
      it "I can't create a viewing party without a date", :vcr do
        ...
        expect(current_path).to eq("/dashboard/movies/550/create")
      end
    end
  end
end
